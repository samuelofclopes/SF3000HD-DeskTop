...
#define DESKTOP_BIN SDCARD_BASE "/cubegm/desktop"   /* standalone desktop launcher */
...
static const AppEntry app_defs[] = {
    ...
    {"desktop", "Desktop", NULL, NULL, DESKTOP_BIN},
};
...
static int is_app_folder_name(const char *name) {
    return name && (!strcasecmp(name, "rockbox") || !strcasecmp(name, "music") ||
                    ...
                    !strcasecmp(name, "ebooks")) || !strcasecmp(name, "desktop");
}
...
static void request_game_launch(const char *core_path, const char *rom_path) {
    /* Standalone: executáveis não podem ser dlopen'd pelo picoarch */
    if (is_standalone_bin(core_path) && access(core_path, F_OK) == 0) {
        request_standalone_launch(core_path, rom_path);
        return;
    }

    FILE *f = fopen(LAUNCH_FILE, "w");   /* ← daqui para baixo: corpo original intocado */
...
static void request_standalone_launch(const char *bin_path, const char *rom_path) {
    ...
    if (strcmp(bin_path, ROCKBOX_BIN) != 0 && strcmp(bin_path, VIDEO_BIN) != 0 &&
        strcmp(bin_path, FROGSHELL_BIN) != 0 && strcmp(bin_path, DESKTOP_BIN) != 0){
